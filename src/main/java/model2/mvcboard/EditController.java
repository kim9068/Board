...
public class EditController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int maxPostSize=5*1024*1024; // 첨부파일 최대크기 5Mbyte로 지지어.
		...
		if(result == 1) {
			...
			response.sendRedirect("/view.do?idx=" + idx); // 상세보기 페이지로 이동
		}
		else {
			JSFunction.alertLocation(response, "오류 발생", "/view.do?idx=" + idx);
		}
